...
import EditorGuidePreview from '../previews/EditorGuidePreviews'
import {
  MdSettings,
  MdVideocam,
  MdImage,
...
export default () =>
  userStore.currentUser.pipe(
    map(({ user }) => {
      ...
      if (role === 'administrator' && email === 'productdevelopment@neemconsulting.com') {
        return S.list()
          .title('Content')
          .items([
            S.divider(),
            S.listItem()
              ...
              .schemaType('editorGuide')
              .child(
                S.documentTypeList('editorGuide')
                  .title('Editor Guide')
                  .child(documentId =>
                    S.document()
